...
def enhancedFeatureExtractorDigit(datum):
    ...
    numLoops = -1   
    stack = util.Stack() 
    zeroPixels = [(x,y) for (x,y) in features.keys() if features[(x,y)] == 0]   
    while len(zeroPixels) > 0:
        #DFS (Depth First Search)
        stack.push(zeroPixels.pop())  
        loopSize = 1
        while not stack.isEmpty():
            x,y = stack.pop()
            possibleNeighbors = [(x+1, y), (x-1,y), (x, y+1), (x, y-1)]
            for neighbor in possibleNeighbors:
                if neighbor in zeroPixels:
                    stack.push(neighbor)
                    zeroPixels.remove(neighbor)
                    loopSize += 1
        if loopSize > 1:
            numLoops += 1
    
    features['moreThanOneLoop'] = int(numLoops > 1)
    features['oneLoop'] = int(numLoops == 1)
    features['noLoops'] = int(numLoops < 1)
...
def enhancedFeatureExtractorPacman(state):
    ...
    for action in state.getLegalActions():
        ...
        capsulesList = successor.getCapsules()
        ghostsStates = successor.getGhostStates()
        ghostsPositions = [ghost.getPosition() for ghost in ghostsStates if ghost.scaredTimer <= 0]
        scaredGhostsPositions = [ghost.getPosition() for ghost in ghostsStates if ghost.scaredTimer > 0]
        pacmanPosition = successor.getPacmanPosition()     
        
        features[action]['capsulesCount'] = len(capsulesList)
        features[action]['ghostsCount'] = len(ghostsPositions)
        features[action]['scaredGhostsCount'] = len(scaredGhostsPositions)
        features[action]['scoreDifference'] = successor.getScore() - state.getScore()
        features[action]['nearestGhostDistance'] = 0
        features[action]['nearestScaredGhostDistance'] = 0
        features[action]['nearestFoodDistance'] = 0
        features[action]['nearestCapsuleDistance'] = 0
  
        if len(ghostsPositions) > 0:
            ghostsDistances = [util.manhattanDistance(pacmanPosition, ghost) for ghost in ghostsPositions]
            features[action]['nearestGhostDistance'] = min(ghostsDistances)

        if len(scaredGhostsPositions) > 0:
            scaredGhostsDistances = [util.manhattanDistance(pacmanPosition, ghost) for ghost in scaredGhostsPositions]
            features[action]['nearestScaredGhostDistance'] = min(scaredGhostsDistances)

        if len(foodList) > 0:
            foodsDistances = [util.manhattanDistance(pacmanPosition, food) for food in foodList]
            features[action]['nearestFoodDistance'] = min(foodsDistances)
        
        if len(capsulesList) > 0:
            capsulesDistances = [util.manhattanDistance(pacmanPosition, capsule) for capsule in capsulesList]
            features[action]['nearestCapsuleDistance'] = min(capsulesDistances)
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    #    features = [f for f in features if type(f) != tuple and features[f]]
    #    print testLabels[i], features 
    #    print rawTestData[i]
